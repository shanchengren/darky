...
class PwActivePost {
	...
	function _setAtt() {
		...
		if ($_POST['flashatt'] && is_array($_POST['flashatt'])) {
			$this->att->transfer($_POST['flashatt'], $_POST['savetoalbum'], $_POST['albumid']);
		}
		PwUpload::upload($this->att);
		if ($idrelate = $this->att->getIdRelate()) {
			foreach ($idrelate as $key => $value) {
				$this->data['content'] = str_replace("[attachment=$value]", "[attachment=$key]", $this->data['content']);
			}
		}
	}
