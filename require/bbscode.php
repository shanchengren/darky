		...
		if ($attachs = $this->getAttachs($pid, $isSelf)) {
			...
			foreach ($attachs as $atype => $value) {
				foreach ($value as $k => $v) {
					if (in_array($k, $aids)) {
						$content = $this->parseContent($content, $atype, $v);
					} else {
						$array[$atype][$k] = $v;
					}
				}
	...
	function analyse(&$attach) {
		...
		if ($attach['type'] == 'img' && $attach['needrvrc'] == 0 && $this->isImgShow) {
			...
			$attach += array(
				...
				'ext' => strtolower(substr(strrchr($at['name'],'.'),1))
			);
		}
		return $atype;
	}

	function parseContent($message, $atype, $att) {
		$html = '';
		switch ($atype) {
		...
		$message = str_replace("[attachment={$att[aid]}]", "<span id=\"att_$att[aid]\" class=\"f12\">".$html.'</span>', $message);
		return $message;
	}
