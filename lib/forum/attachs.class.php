...
class PW_Attachs {
	...
	function getMiniDir($path) {
		global $attachdir,$attachpath,$db_ftpweb,$attach_url;
		if (file_exists($attachdir . '/thumb/mini/' . $path)) return $attachpath . '/thumb/mini/' . $path;
		if ($db_ftpweb) return $db_ftpweb . '/thumb/mini/' . $path;	
		
		if (!is_array($attach_url)) return $attach_url . '/thumb/mini/' . $path;
		return $attach_url[0] . '/thumb/mini/' . $path;
	}

	function getThreadAttachMini($path) {
		global $imgpath;
		if (!$path) return $imgpath . '/imgdel_h200.jpg';
		return $this->getMiniDir($path);
	}
