...
Editor.prototype.plugins.push(function(){
	...
	this.isUBB = true;
	this.textarea.value  && (this.doc.body.innerHTML = ubb2html(this.textarea.value)+'<br/>');
});
Editor.prototype.ubbtoggle = function(){
	...
	if(this.currentMode == 'default'){
		...
		this.textarea.value = this.isUBB ? html2ubb(txt) : html2txt(txt);
		this.textarea.style.display = '';
		this.div.style.display = 'none';
		this.currentMode = 'UBB';
		this.codeModeBtn&&(this.codeModeBtn.className = 'B_onCodeMode');
	}else{
		var txt = this.isUBB ? ubb2html(this.textarea.value) : txt2html(this.textarea.value);
...
function ubb2html(sUBB)
{
	...
	if (para){
		sHtml = '<p style="text-indent: 2em">' + sHtml.split(/\r?\n/g).join('</p><p style="text-indent: 2em">') +'</p>';
	} else {
		sHtml = '<p>' + sHtml.split(/\r?\n/g).join('</p><p>') +'</p>';
	}
...
function html2ubb(sHtml)
{
	...
	if (sUBB.match(/text-indent\:[\s]?2em/)){
		sUBB = '[paragraph]'+sUBB;
	}
	...
	for(i=0;i<3;i++)sUBB=sUBB.replace(/<(div|p)(?:\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,"$2\n");
